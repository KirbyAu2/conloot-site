...
If you only want the short version: we store your email, your saved lists and a push-notification token so the App can do its job. Your lists are private unless you choose to share one — sharing creates a link, and only people who open that link can see the list. You can also use the App **without creating an account** — in that case we store only an app-generated anonymous ID and, if you turn notifications on, a push token; no email or name. We do not run ads, we do not sell your data, and we do not track you across other apps or websites.
